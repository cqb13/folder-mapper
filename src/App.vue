...
<template>
  <div class="flex min-h-screen flex-col bg-zinc-100 text-gray-800">
    <header class="pt-10">
    ...
    <main class="flex-1">
      <section class="flex flex-col items-center">
        ...
        <div>
          <TextInputGroup
            @add-value="handleAddFolder"
            @remove-value="handleRemoveFolder"
            placeholder="Folder Name"
            label="Ignored Folders"
            :listVals="ignoredFolders"
          />
          <TextInputGroup
            @add-value="handleAddFile"
            @remove-value="handleRemoveFile"
            placeholder="File Name/Ending"
            label="Ignored Files/Endings"
            :listVals="ignoredFiles"
          />
        </div>
        <div>
          <CheckBox
            @update-value="handleEndingsChange"
            defaultChecked="checked"
            label="Include Endings"
            tip=".txt, .py, etc..."
          />
          <CheckBox
            @update-value="handleRootChange"
            defaultChecked="checked"
